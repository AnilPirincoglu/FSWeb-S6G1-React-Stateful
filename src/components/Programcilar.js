...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = () => {
    ...
    return programcilar.find((item) => acitveId == item.id).isim;
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: acitveId ? "gold" : 'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => { setActiveId(dev.id)/* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
